...
export const Head = (props: {}) => {
  ...
  if (
    document.location.href.includes("/player") ||
    document.location.href.includes("/pty")
  ) {
    return (
      <>
        <ToastContainer />
    ...
    <div>
      <div className="">
        ...
        <div className="electron-draggable top-0 left-0 sticky z-20 h-[51px] bg-stone-900 flex pt-2 pb-2 gap-10 sm:gap-14 align-baseline justify-center overflow-auto">
          {isSearch ? (
            <SearchRender title={"Search ..."} close={() => setSearch(false)} />
          ) : (
            <></>
          )}
          <div className={buttonStyle} onClick={() => nav("/")}>
            ...
            {showName ? (
              <div className="block text-xs text-center">Discover</div>
            ) : (
              <></>
            )}
          </div>
          <div className={buttonStyle} onClick={() => nav("/browse/movie")}>
            ...
            {showName ? (
              <div className="block text-xs text-center">Movies</div>
            ) : (
              <></>
            )}
          </div>
          <div className={buttonStyle} onClick={() => nav("/browse/tv")}>
            ...
            {showName ? (
              <div className="block text-xs text-center">TV</div>
            ) : (
              <></>
            )}
          </div>
          <div className={buttonStyle} onClick={() => nav("/providers")}>
            ...
            {showName ? (
              <div className="block text-xs text-center">Providers</div>
            ) : (
              <></>
            )}
          </div>
          <div
            ...
            onClick={() =>
              setSearch(!isSearch && !document.location.href.includes("/login"))
            }
          >
            <FaSearch size={iconSize} />
            {showName ? (
              <div className="block text-xs text-center">Search</div>
            ) : (
              <></>
            )}
          </div>
          <div className={buttonStyle} onClick={() => nav("/me")}>
            ...
            {showName ? (
              <div className="block text-xs text-center">Me</div>
            ) : (
              <></>
            )}
          </div>
